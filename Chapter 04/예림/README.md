...
- 후처리
  - 보상 트랜잭션 (트랜잭션 반영된 데이터 되돌림)
  - 데이터 후보정
## HTTP 커넥션 풀
- HTTP 연결도 커넥션 풀을 사용하면 연결 시간을 줄일 수 있어 응답 속도 향상에 도움이 된다.
- HTTP 커넥션 풀 사용 시 고려할 것 3가지
  - HTTP 커넥션 풀의 크기
  - 풀에서 커넥션 가져올 때 대시하는 시간
  - 커넥션 유지 시간 (keep alive)
- 풀의 크기 - 연동할 서비스의 성능에 따라
- 대기 시간 - 대기 시간이 길어지면 전체 응답 시간도 함께 늘어나므로 수 초 이내 짧은 시간을 설정 (ex. 1~5s)
- 커넥션 유지 시간 - 커넥션은 무한정 유지되지 않기 때문에 연동 서비스에 맞춰 적절히 설정
## 연동 서비스 이중화
- 서비스 성장하면 연동 서비스 이중화 고려
- 이중화 고려 시 따져야 할 것 2가지
  - 해당 기능이 서비스의 핵심인지
  - 이중화 비용이 감당 가능한지
