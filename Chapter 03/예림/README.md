...
- 주 DB - 복제 DB 구조를 사용할 때 변경 주 DB를 사용하고 조회는 복제 DB를 사용한다.
  - 읽기 = 복제 DB
  - 쓰기 = 주 DB
- 주 DB에서 변경 직후 복제 DB는 아직 동기화되지 않았을 수 있음 (Replication Lag)
  - 방금 수정한 데이터를 바로 조회하면 없거나 이전 값이 나옴
  - 특히 상태 변경 후 조회는 치명적
- 상태 변경과 직접 연관된 SELECT는 반드시 주 DB에서

### 배치 쿼리 실행 시간 증가
- 배치 쿼리는 데이터가 커질수록 실행시간은 선형이 아니라 폭증
- 특히 위험한 패턴
  ```sql
  GROUP BY
  COUNT
  ...
  대량 기간 조회
  ```
- 한 번에 다 집계하지 말고 시간 / 범위 단위로 나눠서 처리해라
  - 예 : 하루 단위 / 10분 단위 / 이전 결과를 테이블에 누적
- 배치는 에측 가능해야 한다

### 타입이 다른 칼럼 조인 주의
- 문제 상황
  - user.userId -> INT
  - push.receiverId -> VARCHAR
  ```
   user.userId = push.receiverId
  ```
- DB 내부에서 무슨 일이 벌어질까?
  - DB가 타입 변환 수행
  - VARCHAR -> INT (또는 반대)
 ...
 - 해결 방법
   ...
   - 명시적으로 CAST
     ```
     CAST(u.userId AS CHAR) = p.receiverId
     ```
### 테이블 변경은 신중하게
- 위험한 사실
  - MySQL 등은 테이블 변경 시 새 테이블 생성 → 데이터 복사 → 교체
- 대용량 테이블이면 변경 시간 = 수십 분 ~ 수 시간
- 경우에 따라 서비스 중단, 복제 지연, DML 차단


- 컬럼 하나 추가 ≠ 가벼운 작업

- DBA가 있는 회사 → 반드시 상의 필요
...
- 실제 문제
  ...
  - DB CPU가 이미 높다면 커넥션 증가 = 부하 가중
  - 커넥션이 많을수록 컨텍스트 스위칭 증가
  - 전체 처리량 감소
...
- 연결 수 증가는 마지막 카드로
## 실패와 트랜잭션 고려하기
- DB 관련 코드를 작성할 때는 트랜잭션 시작과 종료 경계를 명확히 설정했는지 반드시 확인해야 한다.
